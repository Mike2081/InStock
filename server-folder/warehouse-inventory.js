...
router.post('/warehouses', (req, res) => {
  ...
  const empty = Object.values(warehouse).map(value => value === null || value === "").find(value => value === true);
  if(empty) {
...
module.exports.address = (id) => {
  const warehouse = warehouses.filter(item => item.inventory.find(obj => obj.id == id));
