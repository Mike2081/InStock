...
export default class Warehouses extends Component {
  ...
  render() {
    ...
    for( let i = 0; i < this.state.warehouseInfo.length; i++){
      let block = < WarehouseCard street={ this.state.warehouseInfo[i].address.street}
  ...
  }else{wareBlocks.push('wareBlocks is loading')}
    return (
      <div className="window">
          <button className="window__modal-button" onClick={this.handleClose}>
            <img className="window__modal-plus" src="/Assets/Icons/plus.svg" alt="plus button" />
          </button>
          <Modal handleClose={this.handleClose} fetchWarehouses={this.fetchWarehouses} isOpen={isOpen} />
